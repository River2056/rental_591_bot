...
async def fetch_contents(page):
    page.waitForSelector(
        "#rent-list-app > div > div.list-container-content > div > section.vue-list-rent-content"
    )
    element_handles = await page.JJ(
        "#rent-list-app > div > div.list-container-content > div > section.vue-list-rent-content"
    )
    result = []
...
async def scrap_591_and_send_html_mail(min, max, send_mail):
    ...
    # rent range start
    page.waitForSelector(
        "#rent-list-app > div > div.vue-filter-container > section:nth-child(3) > ul > li.filter-item-input > div > input:nth-child(1)"
    )
    await page.type(
        "#rent-list-app > div > div.vue-filter-container > section:nth-child(3) > ul > li.filter-item-input > div > input:nth-child(1)",
        min if int(min) > 0 else str(config["range-start"]),
    )

    page.waitForSelector(
        "#rent-list-app > div > div.vue-filter-container > section:nth-child(3) > ul > li.filter-item-input > div > input:nth-child(3)"
    )
    await page.type(
        "#rent-list-app > div > div.vue-filter-container > section:nth-child(3) > ul > li.filter-item-input > div > input:nth-child(3)",
        max if int(max) > 0 else str(config["range-end"]),
    )

    page.waitForSelector(
        "#rent-list-app > div > div.vue-filter-container > section:nth-child(3) > ul > li.filter-item-input > div > button"
    )
    await page.click(
        "#rent-list-app > div > div.vue-filter-container > section:nth-child(3) > ul > li.filter-item-input > div > button"
    )
    ...
    while len(total_results) < element_count:
        ...
        # content list
        filter_range_end = int(max) if int(max) > 0 else config["range-end"]
        page_contents = await fetch_contents(page)
        page_contents = list(
            filter(lambda x: x.price_int <= filter_range_end, page_contents)
        )
        total_results.extend(page_contents)

        # next page
        page.waitForSelector(
            "#rent-list-app > div > div.list-container-content > div > section.vue-public-list-page > div > a.pageNext"
        )
        await page.click(
            "#rent-list-app > div > div.list-container-content > div > section.vue-public-list-page > div > a.pageNext"
        )
    ...
    if send_mail:
        send_html_mail(
            ...
            config["email"]["from"]["password"],
            ", ".join(config["email"]["to"]["username"]),
        )
...
def wrapper_func():
    ...
    _ = asyncio.get_event_loop().run_until_complete(
        scrap_591_and_send_html_mail(config["range-start"], config["range-end"], False)
    )
